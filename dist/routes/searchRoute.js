...
const searchQueryInChromaDb_mjs_1 = require("../services/searchQueryInChromaDb.mjs");
...
router.post('/search-similar-content', async (req, res) => {
    try {
        ...
        const result = await (0, searchQueryInChromaDb_mjs_1.searchQueryInChromaDb)(query);
        console.log('Result of searchQueryInChromaDb', result);
        const documentIds = result?.ids.flat();
        //get the contents from mongodb
        const content = await ContentsModel_1.ContentModel.find({ _id: { $in: { documentIds } } });
...
router.post('/search-llm', async (req, res) => {
    try {
        ...
        const llmResponse = await (0, LLM_1.queryLLM)(prompt, content);
